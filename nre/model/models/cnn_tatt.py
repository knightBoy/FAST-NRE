...
from nre.layers.encoder.cnn import CnnEncoder
from nre.layers.selector.type_attention import TypeAttention
from nre.layers.classifier.classifier import Classifier


class CNNTATT(BaseModel):
    def __init__(self, pretrained_word_embeddings, gpu_utils, opt):
        super(CNNTATT, self).__init__(pretrained_word_embeddings, gpu_utils, opt)

        self.beta = opt.beta

        self.embedding_layer = Embedding(pretrained_word_embeddings, opt)
        self.encoder_layer = gpu_utils.module_to_parallel(CnnEncoder(opt))
        self.selector_layer = TypeAttention(opt.hidden_size, gpu_utils.use_gpu, opt)
    ...
    def forward(self, is_train, input_word, input_pos1, input_pos2,
        ...
        encoder_out = self.encoder_layer(embeddings)
